...
class RangeSlider(wx.Panel):
    ...
    def OnMouseLost(self, evt):
        for thumb in self.thumbs.values():
            ...
            thumb.mouse_over = False
        self.Refresh()
        self.PostMouseLostEvent()

    def PostMouseLostEvent(self):
        event = wx.PyCommandEvent(wx.EVT_LEFT_UP.typeId, self.parent.GetId())
        event.SetEventObject(self.parent)
        wx.PostEvent(self.parent.GetEventHandler(), event)
    ...
    def OnPaint(self, evt):
        ...
        dc.DrawRectangle(self.border_width, int(h/2 - track_height/2), w - 2 * self.border_width, track_height)
        # Draw selected range
        if self.IsEnabled():
    ...
    def ResetLimits(self, minValue, maxValue):
        self.min_value = minValue
        self.max_value = maxValue
        self.Refresh()
